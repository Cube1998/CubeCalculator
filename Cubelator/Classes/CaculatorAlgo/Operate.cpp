...
void Operate::FunctionBox(int c)
{
	...
	case 1: {
		...
		Vector1 v1(dimension, x1);
		Vector1 v2(dimension, x2);
		v1.Add(v2);
		cout << "运算结果是: ";
		v1.print();
		break;
	...
	case 7: {
		...
		for (int i = 0; i < d1; i++) {
			int coef, expn;
			cout << "According to the order of the expoment from high to low,input the" << i + 1 << "th (coefficient exponent):" << endl;
			cin >> coef >> expn;
	...
	case 8: {
		string exp;
		cout << "Plz input Four arithmetic expressions" << endl;
		cin >> exp;
		Arithmetic ari(exp);
	...
	case 9: {
		...
		string VarName;
		double Val;
		cout << "Plz input the name of viriable" << endl;
		cin >> VarName;
		cout << "Plz input the expression:" << endl;
		cin >> exp;
		cout << "Plz input the value of viriable:" << endl;
		cin >> Val;
	...
	case 10: {
		cout << "Now input your instructions:" << endl;
		string instruction = "";
